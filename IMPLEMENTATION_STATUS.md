...
- A `0.1.0` English source catalog with 80 messages, including Linux-only status announcements, text-file import labels, provider-profile controls, and the Android vertical-slice UI, covering typed string and integer placeholders, plurals, selects, platform applicability, accessibility context, and per-message source revisions.
...
Assumption: the thirty-nine Linux-only status, text-import, and provider-profile messages use machine-generated draft
...
- `jq` confirmed 66 native message keys in the macOS String Catalog, 45 files in the generated manifest, and 80 canonical entries in every official locale pack; the Linux PO catalogs contain the new status, text-import, and provider-profile keys.
